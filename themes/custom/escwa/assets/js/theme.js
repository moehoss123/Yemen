    ...
    var d,
        ...
        B =
            ...
            (M = (function () {
                ...
                var t = l.prototype;
                return (
                    (t.getOptions = function yourFunction(zed) {
...
this.options || ((this.options = {}), (this.options.hover = ("true" === this.element.getAttribute(_) && !isMobile || "true" === document.body.getAttribute(R)) && "false" !== this.element.getAttribute(_) && !isMobile ));
  					 ...
  					 // Trigger window resize to check on load
...
$(document).ready(function() {     
    $('.nav-link.dropdown-toggle, .dropdown-menu').hover(function(){     
        $('.navbar-container').addClass('backer');    
    },     
    function(){    
        $('.navbar-container').removeClass('backer');  
           
    });

//needed to assign a special div id to the search modal so it can be styled.   
$("#frameModalSearch,#myModal2").on('show.bs.modal', function (e) {
    var that=$(this);
    var id=that.attr('id');
    setTimeout(function(){
...
$('.stickyNavSection').each(function(index, section) {
		var navbarHeight = $('.article-progress').innerHeight();
		var sectionWatcher = scrollMonitor.create(section)			
		var sectionMinusBottomHeadline = scrollMonitor.create(section, {bottom: -1*navbarHeight});
			
		sectionMinusBottomHeadline.stateChange( function() {
			
        if (!sectionWatcher.isInViewport) {
          $('.stickyNavSection').find('.sticky').addClass('hidden'); 
				} else if (sectionMinusBottomHeadline.isInViewport && sectionMinusBottomHeadline.isAboveViewport) {
					$('.stickyNavSection').find('.sticky').removeClass('hidden');
				} else {
          $('.stickyNavSection').find('.sticky').addClass('hidden');
				}
